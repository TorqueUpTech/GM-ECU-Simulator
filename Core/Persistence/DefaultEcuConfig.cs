...
public static class DefaultEcuConfig
{
    ...
    public static void ApplyIfEmpty(VirtualBus bus)
    {
        ...
        // No automatic curated-PID seeding. The ECM/TCM above ship with their own example waveform PIDs; the baseline
        // identity ($1A) + curated live $22 set is an explicit, opt-in editor action (EcuViewModel.SeedDefaultPids).
        // Auto-seeding here (and on every config load) used to resurrect rows the user had deleted, so it was removed.
    }
}
