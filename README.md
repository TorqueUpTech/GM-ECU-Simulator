...
It offers **three ways to connect**: the J2534 path above (the default, for real diagnostic hosts); an **alternative raw-CAN TCP connection** - a localhost socket that lets another program (for example a separately developed gauge or data-logger simulator) join the same virtual bus directly, without going through J2534; or a **physical-hardware bridge** that mirrors the virtual bus onto a real CAN wire through an IXXAT USB-to-CAN or OBDX Pro adapter, so actual hardware (for example an ESP32 CAN-Display) can join the bus. You pick the connection from the mode dropdown; see [Supported protocols](#supported-protocols) below.
...
**Three transports.** Orthogonal to the protocol above is *how* a peer reaches the bus, chosen in the mode dropdown:
...
* **Hardware** - `HardwareCanServer` bridges the virtual bus onto a **physical** CAN wire through an `ICanAdapter` (IXXAT USB-to-CAN V2 via the VCI4 API, or an OBDX Pro scantool over USB / WiFi), so real hardware - e.g. an ESP32 CAN-Display - sees the ECUs' broadcast + diagnostic traffic and can transmit back onto the bus. Opens at the GM normal-mode rate (500 kbit/s, 11-bit); a device picker chooses the adapter. DBC application broadcasts flow over this link (and over TCP). OBDX BLE is not yet enabled. The `Shim/Hardware/` adapter layer is copied from the sibling CAN-Tool project.
...
* **DBC-driven CAN broadcast.** Per ECU, import a `.dbc` to emit unsolicited application frames - the background powertrain traffic a passive logger sees, not request/response diagnostics. The import is scoped (pick the transmitting module, then tick which of its messages to broadcast). Each signal is bit-packed at its DBC start bit / length / byte order with the DBC scale + offset, and mapped to a live `EngineModel` signal (so it tracks the active scenario) or a constant; messages emit at their `GenMsgCycleTime` (editable, free-form ms) while a J2534 host session is open. Edit it in the **CAN Broadcast** section of the ECU editor, and save / load a broadcast set as a standalone `*.dbc.json`.
