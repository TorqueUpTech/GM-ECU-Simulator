...
/// <summary>
/// E40 kernel persona: Handles post-download operations for E40 ECUs.
/// Similar to E38KernelPersona but with 4 MiB flash (vs E38's 2 MiB).
...
/// </summary>
public sealed class E40KernelPersona : KernelPersona
{
    public static readonly E40KernelPersona Instance = new();
    private E40KernelPersona() { }

    public override string Id => "e40kernel";
    public override string DisplayName => "E40 Kernel";

    // E40 flash layout (larger than E38)
    private const uint E40FlashSize = 4 * 1024 * 1024; // 4 MiB
    private const uint E40EraseSectorSize = 64 * 1024; // 64 KiB sectors
    ...
    {
        // Bounds check (E40 is 4 MiB)
        if (address + length > E40FlashSize || length == 0)
        {
            SendNrc(node, ch, 0x35, Nrc.RequestOutOfRange);
            return true;
        }
    ...
    protected override bool HandleWriteMemory(EcuNode node, ReadOnlySpan<byte> usdt, ChannelSession ch)
    {
        ...
        // TODO: Extract address and length from frame data
    ...
    private bool HandleCrc(EcuNode node, ReadOnlySpan<byte> usdt, ChannelSession ch)
    {
        ...
        if (address + length > E40FlashSize || node.State.KernelFlash == null)
        {
    ...
    private bool HandleErase(EcuNode node, ReadOnlySpan<byte> usdt, ChannelSession ch)
    {
        ...
        // Validate alignment and bounds (E40 is 4 MiB)
        if (address % E40EraseSectorSize != 0 || length % E40EraseSectorSize != 0 ||
            address + length > E40FlashSize || node.State.KernelFlash == null)
        {
            SendNrc(node, ch, 0x3D, Nrc.RequestOutOfRange);
            return true;
        }
