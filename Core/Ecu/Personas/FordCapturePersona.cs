...
public sealed class FordCapturePersona : IDiagnosticPersona
{
    ...
    // ---- Iteration knobs: canned responses for specific requests ----
    //
    ...
    // it gives up before any strategy-specific traffic appears. Hard-coding
    // VIN + CalID lets the probe complete and PCMTec moves on to the next
    // request stage - whatever that is, we'll see it in the next log.

    // Mode 09 PID 02 - VIN. SAE J1979 reply: 49 02 NODI <17 VIN bytes>.
    // VIN here is the user-chosen FG-shaped string; PCMTec's parser is
    // expected to accept any 17-char ASCII without further verification.
    private const string Vin17 = "6FPAAAJGCMAY96000";
    private static readonly byte[] Mode09Pid02Reply = BuildMode09Pid02Reply(Vin17);

    // Mode 09 PID 04 - Calibration ID. SAE J1979 specifies CalIDs are 16
    // bytes each, zero-padded if shorter; NODI is the number of items.
    // PCMTec's strategy-verify almost certainly compares this against the
    // loaded strategy's <Strategy> field, which is "HAEE4UY" for the user's
    // FG XR6T bin.
    private const string CalId7 = "HAEE4UY";
    private static readonly byte[] Mode09Pid04Reply = BuildMode09Pid04Reply(CalId7);
    ...
    private static byte[] BuildMode09Pid02Reply(string vin)
    {
    ...
    private static byte[] BuildMode09Pid04Reply(string calId)
    {
        if (calId.Length > 16) throw new ArgumentException("CalID must be <=16 chars", nameof(calId));
        // 49 + 04 + 01 (NODI) + 16 ASCII bytes (zero-padded)
        var reply = new byte[3 + 16];
        reply[0] = 0x49;
        reply[1] = 0x04;
        reply[2] = 0x01;
        System.Text.Encoding.ASCII.GetBytes(calId, 0, calId.Length, reply, 3);
        // bytes 3+calId.Length .. 18 already zero
        return reply;
    }
                        ...
                        DiagnosticStack stack)
    {
        ...
        if (sid == 0x09 && usdt.Length >= 2)
        {
            ...
            if (pid == 0x02)
            {
                node.State.Fragmenter.EnqueueResponse(ch, node.UsdtResponseCanId, Mode09Pid02Reply);
                return true;
            }
            if (pid == 0x04)
            {
                node.State.Fragmenter.EnqueueResponse(ch, node.UsdtResponseCanId, Mode09Pid04Reply);
                return true;
            }
        }
                                         ...
                                         bool isFunctional, byte sid, double nowMs, DiagnosticStack stack)
    {
        ...
        sb.Append("  ");
        // Hex dump of the full USDT payload (including the SID byte for
        // self-contained lines).
        for (int i = 0; i < usdt.Length; i++)
        {
            if (i > 0) sb.Append(' ');
