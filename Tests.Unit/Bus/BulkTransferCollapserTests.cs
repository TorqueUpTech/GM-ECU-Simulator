...
public sealed class BulkTransferCollapserTests
{
    ...
    private static List<string> Run(BulkTransferCollapser c, IEnumerable<(uint CanId, byte[] Payload, string Label)> frames, uint chId = 1)
    {
        ...
        foreach (var (canId, payload, label) in frames)
            c.Process(chId, canId, payload, label, label, isTesterPresent: false, isBroadcast: false, (pretty, _, _, _) => output.Add(pretty));
        return output;
    }
    ...
    public void SeparateChannels_HaveIndependentState()
    {
        ...
        var c = new BulkTransferCollapser();
        var output = new List<string>();
        c.Process(1, 0x7E2, Ff(3110), "ch1-FF",   "ch1-FF",   isTesterPresent: false, isBroadcast: false, (p, _, _, _) => output.Add(p));
        c.Process(1, 0x7E2, Cf(1),    "ch1-CF#1", "ch1-CF#1", isTesterPresent: false, isBroadcast: false, (p, _, _, _) => output.Add(p));
        c.Process(2, 0x7E2, new byte[] { 0x02, 0x3E, 0x00, 0, 0, 0, 0, 0 }, "ch2-SF", "ch2-SF", isTesterPresent: true, isBroadcast: false, (p, _, _, _) => output.Add(p));
        c.Process(1, 0x7E2, Cf(2),    "ch1-CF#2", "ch1-CF#2", isTesterPresent: false, isBroadcast: false, (p, _, _, _) => output.Add(p));
